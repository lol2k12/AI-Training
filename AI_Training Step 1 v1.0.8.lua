--[[
    STA TRUE AI PROJECT
    STAGE 1 / 8 - SEMANTIC WORLD OBSERVER v1.0.8 - POTASSIUM HARDENED
...
print("[STA Stage 1 v1.0.8] FILE_ENTRY_OK")
...
local function __STA_STAGE1_FATAL_HANDLER(err)
    ...
    if type(warn) == "function" then
        warn("[STA Stage 1 v1.0.8] TOP_LEVEL_FATAL | " .. message)
    else
        print("[STA Stage 1 v1.0.8] TOP_LEVEL_FATAL | " .. message)
    end
    return message
end

local __STA_STAGE1_TOP_OK, __STA_STAGE1_TOP_ERR = xpcall(function()
    print("[STA Stage 1 v1.0.8] MODULE_START | TOP_LEVEL_INIT")
    ...
    local VERSION = "Stage1-v1.0.8"
    local SCHEMA = "STA True AI - Semantic World Observer Stage 1 v1.0.8"
    ...
    local function reportRuntimeError(stage, err)
        local msg = "[STA Stage 1 v1.0.8 ERROR] " .. tostring(stage) .. " | " .. tostring(err)
        if type(warn) == "function" then
            warn(msg)
        else
            print(msg)
    ...
    local function protectedStage(stage, fn)
        ...
        if not quiet then
            print("[STA Stage 1 v1.0.8] MODULE_START | " .. stageName)
        end
    ...
    local LOG_FILE = string.format("STA_STAGE1_WORLD_V108_%d.log", SessionUnix)
    local MEMORY_FILE = string.format("STA_STAGE1_WORLD_MEMORY_V108_%d.json", SessionUnix)
    ...
    local function saveMemory()
        if not canWrite then
            ...
            warn("[STA Stage 1 v1.0.8] MEMORY_SAVE_FAIL | " .. msg)
            return false
        end
        ...
        if okEncode and type(encodedOrErr) == "string" then
            ...
            warn(
                "[STA Stage 1 v1.0.8] JSON_HTTP_ENCODER_REJECTED"
                .. " | " .. httpErr
            ...
            if okCustom and type(customOrErr) == "string" then
                ...
                print(
                    "[STA Stage 1 v1.0.8] JSON_CUSTOM_ENCODER_OK"
                ...
                if type(HttpService.JSONDecode) == "function" then
                    ...
                    if okDecode then
                        print("[STA Stage 1 v1.0.8] JSON_CUSTOM_VALIDATION_OK")
                    else
                ...
                for i,sample in ipairs(JsonSanitizeStats.samples) do
                    warn(
                        "[STA Stage 1 v1.0.8] JSON_SANITIZE_SAMPLE | #" .. tostring(i)
        ...
        print("[STA Stage 1 v1.0.8] JSON_ENCODER_USED | " .. tostring(encoderUsed))
        ...
        if JsonSanitizeStats.converted > 0 then
            ...
            print(
                "[STA Stage 1 v1.0.8] JSON_SANITIZE_SUMMARY"
                .. " | Converted=" .. tostring(JsonSanitizeStats.converted)
                .. " | NonFinite=" .. tostring(JsonSanitizeStats.nonFiniteNumbers)
                .. " | Unsupported=" .. tostring(JsonSanitizeStats.unsupportedValues)
                .. " | Cycles=" .. tostring(JsonSanitizeStats.cycles)
        ...
        if verified then
            ...
            print("[STA Stage 1 v1.0.8] MEMORY_SAVE_OK | " .. MEMORY_FILE .. " | Bytes=" .. tostring(#encoded) .. " | Encoder=" .. tostring(encoderUsed))
            return true
        end

        logLine("MEMORY_SAVE_FAIL", MEMORY_FILE, verifyDetail)
        warn("[STA Stage 1 v1.0.8] MEMORY_SAVE_FAIL | " .. MEMORY_FILE .. " | " .. verifyDetail)
        return false
    end
    ...
    local function createMobileControls()
        ...
        MobileStartButton.Activated:Connect(function()
            ...
            print("[STA Stage 1 v1.0.8] MOBILE_INPUT | START")
        ...
        MobileSaveButton.Activated:Connect(function()
            print("[STA Stage 1 v1.0.8] MOBILE_INPUT | SAVE_NOW")
            local ok, result = pcall(saveMemory)
            if not ok then
                reportRuntimeError("MOBILE_SAVE", result)
            elseif result then
                print("[STA Stage 1 v1.0.8] MOBILE_SAVE_OK")
            else
                warn("[STA Stage 1 v1.0.8] MOBILE_SAVE_NOT_VERIFIED")
            end
            updateMobileControlVisuals()
        end)
        ...
        MobileStopButton.Activated:Connect(function()
            if not Running then return end
            print("[STA Stage 1 v1.0.8] MOBILE_INPUT | STOP_SAVE")
        ...
        updateMobileControlVisuals()
        print("[STA Stage 1 v1.0.8] MOBILE_UI_READY")
        return true, "OK"
    end
    ...
    local function startObserver()
        if Running then
            print("[STA Stage 1 v1.0.8] Observer is already running.")
            return
        end

        print("[STA Stage 1 v1.0.8] MODULE_START | POTASSIUM_PREFLIGHT")
        local preflight = {
            {"Players", Players ~= nil},
        ...
        for _, item in ipairs(preflight) do
            ...
            if ok then
                print("[STA Stage 1 v1.0.8] PREFLIGHT_PASS | " .. tostring(name))
            else
                preflightFails = preflightFails + 1
                warn("[STA Stage 1 v1.0.8] PREFLIGHT_WARN | " .. tostring(name))
            end
        end
        print("[STA Stage 1 v1.0.8] MODULE_OK | POTASSIUM_PREFLIGHT | Warnings=" .. tostring(preflightFails))

        local function checkpoint(name)
            print("[STA Stage 1 v1.0.8] START_CHECKPOINT | " .. tostring(name))
        end
        ...
        local function startupBody()
            ...
            LOG_FILE = string.format("STA_STAGE1_WORLD_V108_%d.log", SessionUnix)
            MEMORY_FILE = string.format("STA_STAGE1_WORLD_MEMORY_V108_%d.json", SessionUnix)
            ...
            print("[STA Stage 1 v1.0.8] MODULE_START | INITIAL_SCAN_THREAD")
            local spawnOk, spawnErr = pcall(function()
                safeSpawn(function()
                    ...
                    print("[STA Stage 1 v1.0.8] MODULE_OK | INITIAL_SCAN_THREAD")
                end)
            end)
            if not spawnOk then
            ...
            print(" SEMANTIC WORLD OBSERVER v1.0.8")
        ...
        if not ok then
            ...
            cleanupPartialStart()
            print("[STA Stage 1 v1.0.8] START FAILED - send the START_CHECKPOINT + ERROR lines.")
            return
        end
    end
    ...
    local function stopObserver()
        if not Running then
            print("[STA Stage 1 v1.0.8] Observer is not running.")
            return
        end
        ...
        updateMobileControlVisuals()
        print("[STA Stage 1 v1.0.8] STOPPED")
        print("[STA Stage 1 v1.0.8] LOG: " .. LOG_FILE)
        if memorySaved then
            print("[STA Stage 1 v1.0.8] MEMORY_SAVE_VERIFIED: " .. MEMORY_FILE)
        else
            warn("[STA Stage 1 v1.0.8] MEMORY_NOT_VERIFIED: " .. MEMORY_FILE)
        end
    end
    ...
    print("[STA Stage 1 v1.0.8] BOOTSTRAP_OK")
    ...
    do
        ...
        if okControl and connOrErr then
            ...
            print("[STA Stage 1 v1.0.8] MODULE_OK | CONTROL_BINDING")
        else
    ...
    if DEVICE.TouchEnabled then
        print("[STA Stage 1 v1.0.8] MODULE_START | MOBILE_TOUCH_UI")
        local okMobile, mobileResult, mobileDetail = pcall(createMobileControls)
        if okMobile and mobileResult then
            print("[STA Stage 1 v1.0.8] MODULE_OK | MOBILE_TOUCH_UI")
        else
            reportRuntimeError(
        ...
        print("[STA Stage 1 v1.0.8] MOBILE_TOUCH_UI | SKIPPED | TouchEnabled=false")
    end

    print("============================================================")
    print(" STA TRUE AI PROJECT - STAGE 1 / 8")
    print(" SEMANTIC WORLD OBSERVER v1.0.8 UNIVERSAL")
    ...
    print("[STA Stage 1 v1.0.8] MODULE_OK | TOP_LEVEL_INIT")
end, __STA_STAGE1_FATAL_HANDLER)

if not __STA_STAGE1_TOP_OK then
    print("[STA Stage 1 v1.0.8] ABORTED_DURING_TOP_LEVEL_INIT")
end
