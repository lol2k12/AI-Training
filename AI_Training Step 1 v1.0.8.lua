--[[
    STA TRUE AI PROJECT
    STAGE 1 / 8 - SEMANTIC WORLD OBSERVER v1.1.6 - UNIVERSAL EXECUTOR
...
print("[STA Stage 1 v1.1.6] FILE_ENTRY_OK")
...
local function __STA_STAGE1_FATAL_HANDLER(err)
    ...
    if type(warn) == "function" then
        warn("[STA Stage 1 v1.1.6] TOP_LEVEL_FATAL | " .. message)
    else
        print("[STA Stage 1 v1.1.6] TOP_LEVEL_FATAL | " .. message)
    end
    return message
end

local __STA_STAGE1_TOP_OK, __STA_STAGE1_TOP_ERR = xpcall(function()
    print("[STA Stage 1 v1.1.6] MODULE_START | TOP_LEVEL_INIT")
    ...
    local VERSION = "Stage1-v1.1.6"
    local SCHEMA = "STA True AI - Semantic World Observer Stage 1 v1.1.6"
    ...
    local function reportRuntimeError(stage, err)
        ...
        if t - state.windowStart >= 5 then
            if state.suppressed > 0 then
                warn(
                    "[STA Stage 1 v1.1.6] ERROR_SUPPRESSION_SUMMARY"
                    .. " | Stage=" .. stageText
    ...
    local function protectedStage(stage, fn)
        ...
        if not quiet then
            print("[STA Stage 1 v1.1.6] MODULE_START | " .. stageName)
        end
    ...
    local LOG_FILE = string.format("STA_STAGE1_WORLD_V116_%d.log", SessionUnix)
    local MEMORY_FILE = string.format("STA_STAGE1_WORLD_MEMORY_V116_%d.json", SessionUnix)
    ...
    local function saveMemory()
        if not canWrite then
            ...
            warn("[STA Stage 1 v1.1.6] MEMORY_SAVE_FAIL | " .. msg)
            return false
        end
        ...
        print(
            "[STA Stage 1 v1.1.6] JSON_ENCODER_USED"
        ...
        if not okWrite then
            ...
            warn(
                "[STA Stage 1 v1.1.6] MEMORY_SAVE_FAIL"
                .. " | " .. MEMORY_FILE
        ...
        print(
            "[STA Stage 1 v1.1.6] MEMORY_SAVE_OK"
            .. " | " .. MEMORY_FILE
            .. " | Bytes=" .. tostring(#encoded)
            .. " | Encoder=" .. tostring(encoderUsed)
            .. " | Executor=" .. tostring(EXECUTOR.display)
    ...
    local function saveInitialCheckpoint()
        ...
        print(
            "[STA Stage 1 v1.1.6] INITIAL_CHECKPOINT_OK"
            .. " | " .. MEMORY_FILE
            .. " | Bytes=" .. tostring(#encoded)
            .. " | Encoder=" .. tostring(encoderUsed)
        )
    ...
    local function runMemoryPrune(reason, forceCritical)
        ...
        if worldPruned > 0 or guiPruned > 0 then
            ...
            print(
                "[STA Stage 1 v1.1.6] MEMORY_GUARD_PRUNE"
                .. " | Level=" .. tostring(level)
                .. " | World=" .. tostring(worldPruned)
                .. " | GUI=" .. tostring(guiPruned)
            )
    ...
    local function updateMemoryGuard(force)
        ...
        if newLevel ~= MemoryGuard.level then
            ...
            print(
                "[STA Stage 1 v1.1.6] MEMORY_GUARD"
    ...
    local function createMobileControls()
        ...
        MobileStartButton.Activated:Connect(function()
            ...
            print("[STA Stage 1 v1.1.6] MOBILE_INPUT | START")
        ...
        MobileSaveButton.Activated:Connect(function()
            print("[STA Stage 1 v1.1.6] MOBILE_INPUT | SAVE_NOW")
            local ok, result = pcall(saveMemory)
            if not ok then
                reportRuntimeError("MOBILE_SAVE", result)
            elseif result then
                print("[STA Stage 1 v1.1.6] MOBILE_SAVE_OK")
            else
                warn("[STA Stage 1 v1.1.6] MOBILE_SAVE_NOT_VERIFIED")
            end
            updateMobileControlVisuals()
        end)
        ...
        MobileStopButton.Activated:Connect(function()
            if not Running then return end
            print("[STA Stage 1 v1.1.6] MOBILE_INPUT | STOP_SAVE")
        ...
        updateMobileControlVisuals()
        print("[STA Stage 1 v1.1.6] MOBILE_UI_READY")
        return true, "OK"
    end
    ...
    startObserver = function()
        if Running then
            print("[STA Stage 1 v1.1.6] Observer is already running.")
            return
        end

        print("[STA Stage 1 v1.1.6] MODULE_START | EXECUTOR_PREFLIGHT")
        print(
            "[STA Stage 1 v1.1.6] EXECUTOR_PROFILE"
        ...
        for _, item in ipairs(preflight) do
            ...
            if ok then
                print("[STA Stage 1 v1.1.6] PREFLIGHT_PASS | " .. tostring(name))
            else
                preflightFails = preflightFails + 1
                warn("[STA Stage 1 v1.1.6] PREFLIGHT_WARN | " .. tostring(name))
            end
        end
        print(
            "[STA Stage 1 v1.1.6] MODULE_OK | EXECUTOR_PREFLIGHT"
            .. " | Warnings=" .. tostring(preflightFails)
        )

        local function checkpoint(name)
            print("[STA Stage 1 v1.1.6] START_CHECKPOINT | " .. tostring(name))
        end
        ...
        local function startupBody()
            ...
            LOG_FILE = string.format("STA_STAGE1_WORLD_V116_%d.log", SessionUnix)
            MEMORY_FILE = string.format("STA_STAGE1_WORLD_MEMORY_V116_%d.json", SessionUnix)
            ...
            logLine("PERFORMANCE_COUNTER_SCHEMA", "v1.1.6", "NIL_SAFE=true")
            ...
            print("[STA Stage 1 v1.1.6] MODULE_START | INITIAL_SCAN_THREAD")
            local spawnOk, spawnErr = pcall(function()
                safeSpawn(function()
                    ...
                    print("[STA Stage 1 v1.1.6] MODULE_OK | INITIAL_SCAN_THREAD")
                end)
            end)
            if not spawnOk then
            ...
            print(" SEMANTIC WORLD OBSERVER v1.1.6")
        ...
        if not ok then
            ...
            cleanupPartialStart()
            print("[STA Stage 1 v1.1.6] START FAILED - send the START_CHECKPOINT + ERROR lines.")
            return
        end
    end
    ...
    stopObserver = function()
        if not Running then
            print("[STA Stage 1 v1.1.6] Observer is not running.")
            return
        end
        ...
        updateMobileControlVisuals()
        print("[STA Stage 1 v1.1.6] STOPPED")
        print("[STA Stage 1 v1.1.6] LOG: " .. LOG_FILE)
        if memorySaved then
            print("[STA Stage 1 v1.1.6] MEMORY_SAVE_VERIFIED: " .. MEMORY_FILE)
        else
            warn("[STA Stage 1 v1.1.6] MEMORY_NOT_VERIFIED: " .. MEMORY_FILE)
        end
    end
    ...
    do
        ...
        if startType == "function" and stopType == "function" then
            print(
                "[STA Stage 1 v1.1.6] MOBILE_CALLBACK_BINDING_OK"
                .. " | startObserver=" .. startType
                .. " | stopObserver=" .. stopType
            )
        else
            warn(
                "[STA Stage 1 v1.1.6] MOBILE_CALLBACK_BINDING_FAIL"
                .. " | startObserver=" .. startType
                .. " | stopObserver=" .. stopType
            )
        end
    ...
    print("[STA Stage 1 v1.1.6] BOOTSTRAP_OK")
    ...
    do
        ...
        if okControl and connOrErr then
            ...
            print("[STA Stage 1 v1.1.6] MODULE_OK | CONTROL_BINDING")
        else
    ...
    if DEVICE.TouchEnabled then
        print("[STA Stage 1 v1.1.6] MODULE_START | MOBILE_TOUCH_UI")
        local okMobile, mobileResult, mobileDetail = pcall(createMobileControls)
        if okMobile and mobileResult then
            print("[STA Stage 1 v1.1.6] MODULE_OK | MOBILE_TOUCH_UI")
        else
            reportRuntimeError(
        ...
        print("[STA Stage 1 v1.1.6] MOBILE_TOUCH_UI | SKIPPED | TouchEnabled=false")
    end

    print("============================================================")
    print(" STA TRUE AI PROJECT - STAGE 1 / 8")
    print(" SEMANTIC WORLD OBSERVER v1.1.6 UNIVERSAL EXECUTOR")
    print(
        " EXECUTOR | " .. tostring(EXECUTOR.display)
        .. " | Family=" .. tostring(EXECUTOR.family)
    ...
    print("[STA Stage 1 v1.1.6] MODULE_OK | TOP_LEVEL_INIT")
end, __STA_STAGE1_FATAL_HANDLER)

if not __STA_STAGE1_TOP_OK then
    print("[STA Stage 1 v1.1.6] ABORTED_DURING_TOP_LEVEL_INIT")
end
