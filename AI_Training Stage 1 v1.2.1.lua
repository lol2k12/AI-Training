--[[
    STA TRUE AI PROJECT
    STAGE 1 / 8 - SEMANTIC WORLD OBSERVER v1.2.1 - UNIVERSAL EXECUTOR
...
print("[STA Stage 1 v1.2.1] FILE_ENTRY_OK")
...
local function __STA_STAGE1_FATAL_HANDLER(err)
    ...
    if type(warn) == "function" then
        warn("[STA Stage 1 v1.2.1] TOP_LEVEL_FATAL | " .. message)
    else
        print("[STA Stage 1 v1.2.1] TOP_LEVEL_FATAL | " .. message)
    end
    return message
end

local __STA_STAGE1_TOP_OK, __STA_STAGE1_TOP_ERR = xpcall(function()
    print("[STA Stage 1 v1.2.1] MODULE_START | TOP_LEVEL_INIT")
    ...
    local EXECUTOR = {
        ...
        -- EVENT_DRIVEN for executors that tolerate many Instance signals.
        -- XENO uses POLLING_ONLY: no per-object AttributeChanged/Changed/
        -- Prompt property connections are created.
        worldObservationMode = "EVENT_DRIVEN",
    ...
    local function detectExecutor()
        ...
        if EXECUTOR.family == "XENO" then
            ...
            EXECUTOR.worldObservationMode = "POLLING_ONLY"
        else
            EXECUTOR.guiMode = "FULL_PLAYERGUI_BUDGETED"
            EXECUTOR.worldObservationMode = "EVENT_DRIVEN"
        end
    end

    detectExecutor()

    local VERSION = "Stage1-v1.2.1"
    local SCHEMA = "STA True AI - Semantic World Observer Stage 1 v1.2.1"
    ...
    local CONFIG = {
        ...
        -- Signal-free XENO polling. Uses the existing frame/time budgets.
        XenoPollingStateInterval = 0.65,
    }
    ...
    local function reportRuntimeError(stage, err)
        ...
        if t - state.windowStart >= 5 then
            if state.suppressed > 0 then
                warn(
                    "[STA Stage 1 v1.2.1] ERROR_SUPPRESSION_SUMMARY"
                    .. " | Stage=" .. stageText
    ...
    local function protectedStage(stage, fn)
        ...
        if not quiet then
            print("[STA Stage 1 v1.2.1] MODULE_START | " .. stageName)
        end
    ...
    R.Running = false
    R.SessionStartClock = nil
    R.SessionUnix = safeUnixTime()

    R.LOG_FILE =
        string.format("STA_STAGE1_WORLD_V121_%d.log", R.SessionUnix)
    R.MEMORY_FILE =
        string.format(
            "STA_STAGE1_WORLD_MEMORY_V121_%d.json",
            R.SessionUnix
        )

    R.canWrite =
        CONFIG.SaveToFile and type(writefile) == "function"
    R.canAppend =
        CONFIG.SaveToFile and type(appendfile) == "function"

    R.LogBuffer = {}
    R.Connections = {}
    R.MainHeartbeatConnection = nil
    R.CharacterConnection = nil
    R.ControlConnection = nil
    R.MobileUIConnection = nil
    R.MobileGui = nil
    R.MobileStatusLabel = nil
    R.MobileStartButton = nil
    R.MobileSaveButton = nil
    R.MobileStopButton = nil
    ...
    R.GuiRuntimeByInstance =
        setmetatable({}, {__mode = "k"})

    R.WorldInstanceByRuntimeId =
        setmetatable({}, {__mode = "v"})
    R.GuiInstanceByRuntimeId =
        setmetatable({}, {__mode = "v"})

    R.ObjectConnectionsByRuntimeId = {}
    R.GuiConnectionsByRuntimeId = {}

    R.WorldObjects = {}
    R.GuiObjects = {}
    R.SemanticCandidates = {}
    R.ResourceClusters = {}
    R.BaseInfrastructure = {}
    R.WorldInfrastructure = {}
    R.LootContainers = {}
    R.Interactables = {}
    R.UnknownObjects = {}
    R.SignatureHistory = {}

    R.ArchivedWorldEntities = {}
    R.ArchivedWorldOrder = {}
    ...
    R.LastObjectStateScan = 0
    R.LastClusterScan = 0
    R.LastTargetedGuiPoll = 0
    ...
    R.LastContextScan = 0
    R.LastAutosave = 0
    R.LastConsoleStatus = 0

    R.DynamicWorldQueue = {}
    R.DynamicWorldHead = 1
    R.DynamicWorldQueued =
        setmetatable({}, {__mode = "k"})
    ...
    R.PeriodicRefreshList = {}
    R.PeriodicRefreshIndex = 1

    R.ResourceClusterDirty = true
    R.LastClusterVerification = 0

    R.FpsEma = 60
    R.LastHeartbeatAt = nil
    R.StreamingBurstPeak = 0
    ...
    local function elapsed()
        if not R.SessionStartClock then return 0 end
        return now() - R.SessionStartClock
    end
    ...
    logLine = function(kind, ...)
        if not R.Running and kind ~= "CONTROL" then return end
        ...
        if #R.LogBuffer >= CONFIG.LogFlushSize then
            local text = table.concat(R.LogBuffer, "\n") .. "\n"
            if R.canAppend then
                pcall(function() appendfile(R.LOG_FILE, text) end)
            elseif R.canWrite then
                local existing = ""
                if type(readfile) == "function" and type(isfile) == "function" then
                    pcall(function()
                        if isfile(R.LOG_FILE) then existing = readfile(R.LOG_FILE) end
                    end)
                end
                pcall(function() writefile(R.LOG_FILE, existing .. text) end)
            end
            R.LogBuffer = {}
        end
    end
    ...
    local function connect(conn)
        R.Connections[#R.Connections + 1] = conn
        return conn
    end

    local function connectOwned(runtimeId, conn)
        local list = R.ObjectConnectionsByRuntimeId[runtimeId]
        if not list then
            list = {}
            R.ObjectConnectionsByRuntimeId[runtimeId] = list
        end
        list[#list + 1] = conn
        return conn
    end
    ...
    local function connectGuiOwned(runtimeId, conn)
        local list = R.GuiConnectionsByRuntimeId[runtimeId]
        if not list then
            list = {}
            R.GuiConnectionsByRuntimeId[runtimeId] = list
        end
        list[#list + 1] = conn
        return conn
    end
    ...
    local function disconnectOwned(map, runtimeId)
        ...
        R.MemoryGuard.ownedConnectionsDisconnected =
            (R.MemoryGuard.ownedConnectionsDisconnected or 0) + disconnected
        R.Statistics.memoryOwnedConnectionsDisconnected =
            (R.Statistics.memoryOwnedConnectionsDisconnected or 0) + disconnected
    ...
    local function disconnectAll()
        ...
        disconnectAllOwned(R.ObjectConnectionsByRuntimeId)
        disconnectAllOwned(R.GuiConnectionsByRuntimeId)
    end
    ...
    local function recomputeBaseRegion()
        local samples = R.BaseRegionModel.recentSamples
        if #samples == 0 then
            R.BaseRegionModel.center = nil
            R.BaseRegionModel.observedRadius = 0
            R.BaseRegionModel.effectiveRadius = 0
            R.BaseRegionModel.confidence = 0
            return
        end
        ...
        R.BaseRegionModel.center = vectorToTable(center)
        R.BaseRegionModel.observedRadius = round(radius, 2)
        R.BaseRegionModel.effectiveRadius = round(
            math.max(CONFIG.BaseRegionMinimumRadius, radius + CONFIG.BaseRegionPadding),
            2
        )
        R.BaseRegionModel.confidence = round(
            math.min(0.97, 0.20 + (math.min(R.BaseRegionModel.sampleCount, 60) / 60) * 0.77),
            3
        )
        R.BaseRegionModel.lastUpdatedAt = elapsed()
    end
    ...
    local function addBaseRegionSample(pos)
        ...
        R.BaseRegionModel.sampleCount = R.BaseRegionModel.sampleCount + 1
        R.Statistics.baseRegionSamples = (R.Statistics.baseRegionSamples or 0) + 1
        recomputeBaseRegion()
    end

    local function baseRegionStatusForPosition(pos)
        if typeof(pos) ~= "Vector3" then return nil, 0, nil end
        if R.BaseRegionModel.sampleCount < CONFIG.BaseRegionMinSamples then
            return nil, R.BaseRegionModel.confidence or 0, nil
        end
        if not R.BaseRegionModel.center then
            return nil, R.BaseRegionModel.confidence or 0, nil
        end

        local c = R.BaseRegionModel.center
        local center = Vector3.new(c.x, c.y, c.z)
        local d = distance(pos, center)
        local inside = d <= (R.BaseRegionModel.effectiveRadius or CONFIG.BaseRegionMinimumRadius)

        return inside, R.BaseRegionModel.confidence or 0, round(d, 2)
    end
    ...
    local function nextObjectId()
        R.ObjectCounter = R.ObjectCounter + 1
        return string.format("O%06d", R.ObjectCounter)
    end

    local function nextGuiId()
        R.GuiCounter = R.GuiCounter + 1
        return string.format("G%06d", R.GuiCounter)
    end

    local function registerSemanticRecord(runtimeId, candidates, record)
        if not candidates or #candidates == 0 then return end
        R.SemanticCandidates[runtimeId] = candidates

        for _,c in ipairs(candidates) do
            R.Statistics.semanticCandidates = R.Statistics.semanticCandidates + 1
            R.Statistics.byCategory[c.category] = (R.Statistics.byCategory[c.category] or 0) + 1

            if isInfrastructureCategory(c.category) then
                R.BaseInfrastructure[c.category] = R.BaseInfrastructure[c.category] or {}
                R.BaseInfrastructure[c.category][runtimeId] = true
                if record and record.isCanonicalSemanticEntity and record.position then
                    R.BaseRegionModel.anchorIds[runtimeId] = true
                end
            elseif isWorldInfrastructureCategory(c.category) then
                R.WorldInfrastructure[c.category] = R.WorldInfrastructure[c.category] or {}
                R.WorldInfrastructure[c.category][runtimeId] = true
            elseif isLootCategory(c.category) then
                R.LootContainers[c.category] = R.LootContainers[c.category] or {}
                R.LootContainers[c.category][runtimeId] = true
            end
            ...
            then
                R.Interactables[runtimeId] = true
            end
        end
    end
    ...
    local function unregisterSemanticRecord(runtimeId, candidates)
        ...
        for _,c in ipairs(candidates) do
            R.Statistics.semanticCandidates = math.max(0, (R.Statistics.semanticCandidates or 0) - 1)
            if R.Statistics.byCategory[c.category] then
                R.Statistics.byCategory[c.category] = math.max(0, R.Statistics.byCategory[c.category] - 1)
            end

            if isInfrastructureCategory(c.category) and R.BaseInfrastructure[c.category] then
                R.BaseInfrastructure[c.category][runtimeId] = nil
            elseif isWorldInfrastructureCategory(c.category) and R.WorldInfrastructure[c.category] then
                R.WorldInfrastructure[c.category][runtimeId] = nil
            elseif isLootCategory(c.category) and R.LootContainers[c.category] then
                R.LootContainers[c.category][runtimeId] = nil
            end
        end

        R.Interactables[runtimeId] = nil
        R.BaseRegionModel.anchorIds[runtimeId] = nil
        R.SemanticCandidates[runtimeId] = nil
    end
    ...
    local function refreshSemanticClassification(obj, runtimeId, reason)
        local rec = R.WorldObjects[runtimeId]
        ...
        R.Statistics.semanticReclassifications =
            (R.Statistics.semanticReclassifications or 0) + 1
    ...
    local function maybeUnknown(runtimeId, record)
        ...
        if tableCount(R.UnknownObjects) >= CONFIG.MaxUnknownObjects then return end
        if tableCount(record.attributes) > 0 or #(record.tags or {}) > 0 or record.class == "Model" or record.class == "Tool" or record.class == "ProximityPrompt" then
            R.UnknownObjects[runtimeId] = true
        end
    end

    local function monitorPrompt(prompt, runtimeId)
        if not prompt:IsA("ProximityPrompt") then return end
        R.Statistics.promptDiscovered = R.Statistics.promptDiscovered + 1

        if EXECUTOR.worldObservationMode == "POLLING_ONLY" then
            return
        end
        ...
        for _,prop in ipairs(properties) do
            connectOwned(runtimeId, prompt:GetPropertyChangedSignal(prop):Connect(function()
                ...
                R.Statistics.promptChanges = R.Statistics.promptChanges + 1
                ...
                if prop == "ActionText" or prop == "ObjectText" then
                    ...
                    if owner and owner ~= prompt then
                        local ownerId = R.RuntimeByInstance[owner]
                        if ownerId then
    ...
    local function monitorValueObject(obj, runtimeId)
        ...
        connectOwned(runtimeId, obj.Changed:Connect(function(value)
            ...
            rec.value = newValue
            rec.lastSeenAt = elapsed()
            R.Statistics.valueChanges = R.Statistics.valueChanges + 1
            ...
            if DEVICE.Mobile and type(old) == "number" and type(newValue) == "number" then
                ...
                if t - rec._lastValueLogAt < (CONFIG.NumericChangeLogInterval * (R.MemoryGuard.logMultiplier or 1)) then
                    shouldLog = false
                    R.Statistics.throttledNumericLogs = (R.Statistics.throttledNumericLogs or 0) + 1
                else
    ...
    local function monitorAttributes(obj, runtimeId)
        ...
        connectOwned(runtimeId, obj.AttributeChanged:Connect(function(attributeName)
            ...
            rec.lastSeenAt = elapsed()
            R.Statistics.attributeChanges = R.Statistics.attributeChanges + 1
            ...
            if DEVICE.Mobile and type(old) == "number" and type(newValue) == "number" then
                ...
                if t - last < (CONFIG.NumericChangeLogInterval * (R.MemoryGuard.logMultiplier or 1)) then
                    shouldLog = false
                    R.Statistics.throttledNumericLogs = (R.Statistics.throttledNumericLogs or 0) + 1
                else
    ...
    local function registerWorldObject(obj, preexisting)
        if not R.Running or not obj or not obj.Parent then return nil end
        if R.RuntimeByInstance[obj] then return R.RuntimeByInstance[obj] end
        if not shouldTrackObject(obj) then return nil end

        local snapshot = snapshotObject(obj)
        local runtimeId = nextObjectId()
        R.RuntimeByInstance[obj] = runtimeId
        R.WorldInstanceByRuntimeId[runtimeId] = obj

        local previous = R.SignatureHistory[snapshot.signature]
        local reobservedFrom = nil
        if previous and previous.removedAt then
            reobservedFrom = previous.runtimeId
            R.Statistics.objectsReobserved = R.Statistics.objectsReobserved + 1
        end
        ...
        record.importantForMovement =
            classificationImportant(record.semanticCandidates)
            or obj:IsA("Tool")
            or obj:IsA("Model")

        local xenoPollImportant =
            EXECUTOR.worldObservationMode == "POLLING_ONLY"
            and (
                obj:IsA("ProximityPrompt")
                or obj:IsA("ValueBase")
                or tableCount(record.attributes or {}) > 0
                or classificationImportant(record.semanticCandidates)
            )

        record.periodicRefresh =
            record.importantForMovement
            or #(record.tags or {}) > 0
            or xenoPollImportant

        record.signalFreePolling = xenoPollImportant == true

        if record.periodicRefresh then
            -- runtimeId only: no strong Instance reference.
            R.PeriodicRefreshList[#R.PeriodicRefreshList + 1] = {
                runtimeId = runtimeId,
            }
        end
        ...
        R.WorldObjects[runtimeId] = record
        R.SignatureHistory[snapshot.signature] = {runtimeId = runtimeId, removedAt = nil}

        R.Statistics.objectsObserved = R.Statistics.objectsObserved + 1
        R.Statistics.byClass[obj.ClassName] = (R.Statistics.byClass[obj.ClassName] or 0) + 1
        if record.isCanonicalSemanticEntity then
            R.Statistics.canonicalEntities = (R.Statistics.canonicalEntities or 0) + 1
        end
        if preexisting then
            R.Statistics.objectsPreexistingAtStart = R.Statistics.objectsPreexistingAtStart + 1
        else
            R.Statistics.objectsObservedAfterStart = R.Statistics.objectsObservedAfterStart + 1
        end
    ...
    local function markWorldObjectRemoved(obj)
        local runtimeId = R.RuntimeByInstance[obj]
        if not runtimeId then return end
        local rec = R.WorldObjects[runtimeId]
        if not rec or rec.removedAt then return end
        rec.removedAt = elapsed()
        rec.lastSeenAt = elapsed()
        R.Statistics.objectsRemoved = R.Statistics.objectsRemoved + 1
        ...
        if rec.signature then
            R.SignatureHistory[rec.signature] = {runtimeId = runtimeId, removedAt = rec.removedAt}
        end
        ...
        disconnectOwned(R.ObjectConnectionsByRuntimeId, runtimeId)
        R.WorldInstanceByRuntimeId[runtimeId] = nil
        R.RuntimeByInstance[obj] = nil

        logLine("OBJECT_LOST_FROM_OBSERVATION", runtimeId, rec.class, rec.name, rec.path)
    end

    local function tablesScalarEqual(a, b)
        a = a or {}
        b = b or {}
    ...
    local function pollSignalFreeState(obj, runtimeId, rec)
        if EXECUTOR.worldObservationMode ~= "POLLING_ONLY" then
            return
        end
        ...
        if obj:IsA("ValueBase") then
            ...
            if okValue then
                local newValue = safeScalar(rawValue)
                local oldValue = rec.value

                if oldValue ~= newValue then
                    rec.value = newValue
                    R.Statistics.valueChanges =
                        (R.Statistics.valueChanges or 0) + 1
        ...
        -- Attributes
        -- --------------------------------------------------------
        local newAttributes = getAttributes(obj)
        local oldAttributes = rec.attributes or {}

        if not tablesScalarEqual(oldAttributes, newAttributes) then
            local changedIdentityAttribute = false

            for key,newValue in pairs(newAttributes) do
                local oldValue = oldAttributes[key]
                if oldValue ~= newValue then
                    R.Statistics.attributeChanges =
                        (R.Statistics.attributeChanges or 0) + 1

                    logLine(
                        "OBJECT_ATTRIBUTE_CHANGE_POLL",
                        runtimeId,
                        safeFullName(obj),
                        tostring(key),
                        tostring(oldValue),
                    ...
                    if key == "ToolType"
                        ...
                        or key == "Type"
                    then
                        changedIdentityAttribute = true
                    end
                end
            end

            for key,oldValue in pairs(oldAttributes) do
                if newAttributes[key] == nil then
                    R.Statistics.attributeChanges =
                        (R.Statistics.attributeChanges or 0) + 1
            ...
            rec.lastAttributes = shallowCopy(newAttributes)

            if changedIdentityAttribute then
                refreshSemanticClassification(
                    obj,
                    runtimeId,
                    "EXPLICIT_TYPE_ATTRIBUTE_POLL"
                )
            end
        end
        ...
        -- --------------------------------------------------------
        if obj:IsA("ProximityPrompt") then
            local fresh = promptSnapshot(obj)
            local old = rec.prompt or {}

            local changed =
                old.actionText ~= fresh.actionText
                or old.objectText ~= fresh.objectText
                or old.enabled ~= fresh.enabled
                or old.holdDuration ~= fresh.holdDuration
                or old.maxActivationDistance
                    ~= fresh.maxActivationDistance
                or old.requiresLineOfSight
                    ~= fresh.requiresLineOfSight
                or old.keyboardKeyCode
                    ~= fresh.keyboardKeyCode
                or old.gamepadKeyCode
                    ~= fresh.gamepadKeyCode
                or old.clickablePrompt
                    ~= fresh.clickablePrompt
            ...
            if changed then
                ...
                rec.prompt = fresh
                R.Statistics.promptChanges =
                    (R.Statistics.promptChanges or 0) + 1

                logLine(
                    "PROMPT_CHANGED_POLL",
                    runtimeId,
                    safeFullName(obj),
                    "Action=" .. tostring(fresh.actionText),
                    "Object=" .. tostring(fresh.objectText),
                ...
                if identityChanged then
                    refreshSemanticClassification(
                        obj,
                        runtimeId,
                        "PROMPT_IDENTITY_POLL"
                    )

                    local owner = semanticOwnerFor(obj)
                    if owner and owner ~= obj then
                        local ownerId = R.RuntimeByInstance[owner]
                        if ownerId then
                            refreshSemanticClassification(
                                owner,
                                ownerId,
                                "CHILD_PROMPT_IDENTITY_POLL"
                            )
                        end
                    end
                end
    ...
    local function refreshWorldObject(obj, runtimeId)
        ...
        if pos then
            if rec.lastPositionVector then
                ...
                if rec.importantForMovement and d >= CONFIG.ImportantMoveDistance then
                    R.Statistics.objectsMoved = R.Statistics.objectsMoved + 1
                    local okResourceCandidate, isResource = pcall(recordHasResourceCandidate, rec)
                    if okResourceCandidate and isResource then
                        R.ResourceClusterDirty = true
                    elseif not okResourceCandidate then
        ...
        pollSignalFreeState(obj, runtimeId, rec)
    ...
    local function shouldTrackGui(obj)
        ...
        if R.MobileGui and (obj == R.MobileGui or obj:IsDescendantOf(R.MobileGui)) then
            return false
        end
    ...
    local function registerGui(obj, preexisting)
        if not R.Running or not obj or not obj.Parent or not shouldTrackGui(obj) then return end
        if R.GuiRuntimeByInstance[obj] then return end

        local runtimeId = nextGuiId()
        R.GuiRuntimeByInstance[obj] = runtimeId
        R.GuiInstanceByRuntimeId[runtimeId] = obj
        ...
        rec.removedAt = nil
        R.GuiObjects[runtimeId] = rec
        R.Statistics.guiObserved = R.Statistics.guiObserved + 1
        ...
        local function propertyWatcher(prop, key)
            connectGuiOwned(runtimeId, obj:GetPropertyChangedSignal(prop):Connect(function()
                if not R.Running then return end
                local r = R.GuiObjects[runtimeId]
                ...
                R.Statistics.guiChanges = R.Statistics.guiChanges + 1

                local shouldLog = true
                if DEVICE.Mobile and prop == "Text" then
                    local t = now()
                    r._lastTextLogAt = r._lastTextLogAt or 0
                    if t - r._lastTextLogAt < (CONFIG.GuiTextLogInterval * (R.MemoryGuard.logMultiplier or 1)) then
                        shouldLog = false
                        R.Statistics.throttledGuiTextLogs = (R.Statistics.throttledGuiTextLogs or 0) + 1
                    else
                        r._lastTextLogAt = t
                    end
                end
    ...
    local function markGuiRemoved(obj)
        local runtimeId = R.GuiRuntimeByInstance[obj]
        if not runtimeId then return end
        local rec = R.GuiObjects[runtimeId]
        if rec and not rec.removedAt then
            rec.removedAt = elapsed()
            rec.lastSeenAt = elapsed()

            disconnectOwned(R.GuiConnectionsByRuntimeId, runtimeId)
            R.GuiInstanceByRuntimeId[runtimeId] = nil
            R.GuiRuntimeByInstance[obj] = nil
    ...
    local function currentResourcePoints()
        local byCategory = {}
        for runtimeId, rec in pairs(R.WorldObjects) do
    ...
    local function updateResourceClusters()
        ...
        for category, points in pairs(resourcePoints) do
            ...
            for _,cluster in ipairs(clusters) do
                ...
                local rec = R.ResourceClusters[key]
                local memberIds = {}
                for _,m in ipairs(cluster.members) do memberIds[#memberIds + 1] = m.runtimeId end
                table.sort(memberIds)

                if not rec then
                    R.ClusterCounter = R.ClusterCounter + 1
                    rec = {
                        clusterId = string.format("C%05d", R.ClusterCounter),
                        key = key,
                        category = category,
                        firstSeenAt = elapsed(),
                        lastSeenAt = elapsed(),
                    ...
                    R.ResourceClusters[key] = rec
                    R.Statistics.resourceClustersCreated = R.Statistics.resourceClustersCreated + 1
                ...
                if rec.observations > 1 and (changedCount or changedMembers) then
                    R.Statistics.resourceClusterUpdates = R.Statistics.resourceClusterUpdates + 1
                    logLine(
    ...
    local function updateContext()
        ...
        if R.CurrentContext.phase ~= nil and R.CurrentContext.phase ~= phase then
            logLine(
                "TIME_PHASE_CHANGE",
                tostring(R.CurrentContext.phase),
                "->",
                tostring(phase),
                "Day=" .. tostring(day),
        ...
        if R.CurrentContext.nearBase ~= nil and R.CurrentContext.nearBase ~= nearBase then
            logLine(
                "PLAYER_NEAR_BASE_CHANGE",
                tostring(R.CurrentContext.nearBase),
                "->",
                tostring(nearBase)
            )
        end

        if R.CurrentContext.day ~= nil and day ~= nil and R.CurrentContext.day ~= day then
            logLine(
                "DAY_COUNTER_CHANGE",
                tostring(R.CurrentContext.day),
                "->",
                tostring(day),
        ...
        R.CurrentContext.phase = phase
        R.CurrentContext.nearBase = nearBase
        R.CurrentContext.playerPosition = vectorToTable(pos)
        R.CurrentContext.day = day
        R.CurrentContext.daySource = daySource
        R.CurrentContext.highestDayRecord = highestDay
    end
    ...
    local function sanitizeWorldObjects()
        local out = {}
        for id,rec in pairs(R.WorldObjects) do
            local copy = {}
        ...
        while i <= len do
            ...
            if b1 < 0x80 then
                ...
                if cp then
                    ...
                    out[#out + 1] = "\\uFFFD"
                    R.Statistics.invalidUtf8BytesReplaced =
                        (R.Statistics.invalidUtf8BytesReplaced or 0) + 1
                    i = i + 1
                end
            end
        end
    ...
    local function buildMemory()
        return {
            ...
            targetedGuiState = R.TargetedGuiState,
            currentContext = R.CurrentContext,
            statistics = R.Statistics,
            worldObjects = sanitizeWorldObjects(),
            semanticCandidates = R.SemanticCandidates,
            resourceClusters = R.ResourceClusters,
            archivedWorldEntities = R.ArchivedWorldEntities,
            memoryGuard = R.MemoryGuard,
            baseRegionModel = R.BaseRegionModel,
            baseInfrastructure = indexSetToArray(R.BaseInfrastructure),
            worldInfrastructure = indexSetToArray(R.WorldInfrastructure),
            lootContainers = indexSetToArray(R.LootContainers),
            interactables = idSetToArray(R.Interactables),
            unknownObjects = idSetToArray(R.UnknownObjects),
            ...
            semanticRules = {
                ...
                coreGuiRule = "PLAYERGUI_TARGETED_ON_XENO_NO_COREGUI_LOCALE_MODULE_TRAVERSAL",
                xenoWorldRule = "NO_PER_OBJECT_RBXSCRIPT_SIGNAL_CONNECTIONS_USE_BUDGETED_POLLING",
            },
        }
    end
    ...
    local function saveMemory()
        if not R.canWrite then
            ...
            warn("[STA Stage 1 v1.2.1] MEMORY_SAVE_FAIL | " .. msg)
            return false
        end
        ...
        print(
            "[STA Stage 1 v1.2.1] JSON_ENCODER_USED"
        ...
        local okWrite, verifyDetail =
            writeVerifiedFile(R.MEMORY_FILE, encoded)

        if not okWrite then
            logLine("MEMORY_SAVE_FAIL", R.MEMORY_FILE, tostring(verifyDetail))
            warn(
                "[STA Stage 1 v1.2.1] MEMORY_SAVE_FAIL"
                .. " | " .. R.MEMORY_FILE
        ...
        print(
            "[STA Stage 1 v1.2.1] MEMORY_SAVE_OK"
            .. " | " .. R.MEMORY_FILE
            .. " | Bytes=" .. tostring(#encoded)
            .. " | Encoder=" .. tostring(encoderUsed)
            .. " | Executor=" .. tostring(EXECUTOR.display)
    ...
    local function saveInitialCheckpoint()
        ...
        print(
            "[STA Stage 1 v1.2.1] INITIAL_CHECKPOINT_OK"
            .. " | " .. R.MEMORY_FILE
            .. " | Bytes=" .. tostring(#encoded)
            .. " | Encoder=" .. tostring(encoderUsed)
        )
    ...
    local function archiveSemanticRecord(runtimeId, rec)
        ...
        if not R.ArchivedWorldEntities[key] then
            R.ArchivedWorldOrder[#R.ArchivedWorldOrder + 1] = key
        end

        R.ArchivedWorldEntities[key] = {
            runtimeId = runtimeId,
        ...
        R.MemoryGuard.semanticArchivesCreated =
            (R.MemoryGuard.semanticArchivesCreated or 0) + 1
        R.Statistics.memorySemanticArchivesCreated =
            (R.Statistics.memorySemanticArchivesCreated or 0) + 1

        while #R.ArchivedWorldOrder > CONFIG.MaxArchivedSemanticEntities do
            local oldestKey = table.remove(R.ArchivedWorldOrder, 1)
            R.ArchivedWorldEntities[oldestKey] = nil
        end
    end

    local function removeIdFromSemanticIndexes(runtimeId, candidates)
        for _,c in ipairs(candidates or {}) do
            if R.BaseInfrastructure[c.category] then
                R.BaseInfrastructure[c.category][runtimeId] = nil
            end
            if R.WorldInfrastructure[c.category] then
                R.WorldInfrastructure[c.category][runtimeId] = nil
            end
            if R.LootContainers[c.category] then
                R.LootContainers[c.category][runtimeId] = nil
            end
        end

        R.Interactables[runtimeId] = nil
        R.UnknownObjects[runtimeId] = nil
        R.BaseRegionModel.anchorIds[runtimeId] = nil
    end
    ...
    local function pruneRemovedWorldRecords(ageSeconds, maxCount)
        ...
        for i = 1, math.min(maxCount, #candidates) do
            ...
            if rec and rec.removedAt then
                ...
                R.SemanticCandidates[runtimeId] = nil
                R.WorldObjects[runtimeId] = nil
                R.WorldInstanceByRuntimeId[runtimeId] = nil
                R.ObjectConnectionsByRuntimeId[runtimeId] = nil
                removed = removed + 1
            end
        end

        if removed > 0 then
            R.MemoryGuard.worldRecordsPruned =
                (R.MemoryGuard.worldRecordsPruned or 0) + removed
            R.Statistics.memoryWorldRecordsPruned =
                (R.Statistics.memoryWorldRecordsPruned or 0) + removed
        end
    ...
    local function pruneRemovedGuiRecords(ageSeconds, maxCount)
        ...
        for runtimeId,rec in pairs(R.GuiObjects) do
            if rec.removedAt and (t - rec.removedAt) >= ageSeconds then
                candidates[#candidates + 1] = {
                    runtimeId = runtimeId,
                    removedAt = rec.removedAt,
        ...
        for i = 1, math.min(maxCount, #candidates) do
            ...
            local rec = R.GuiObjects[runtimeId]
            if rec and rec.removedAt then
                R.GuiObjects[runtimeId] = nil
                R.GuiInstanceByRuntimeId[runtimeId] = nil
                R.GuiConnectionsByRuntimeId[runtimeId] = nil
                removed = removed + 1
            end
        end

        if removed > 0 then
            R.MemoryGuard.guiRecordsPruned =
                (R.MemoryGuard.guiRecordsPruned or 0) + removed
            R.Statistics.memoryGuiRecordsPruned =
                (R.Statistics.memoryGuiRecordsPruned or 0) + removed
        end
    ...
    local function compactPeriodicRefreshList()
        ...
        for _,item in ipairs(R.PeriodicRefreshList) do
            if item and item.runtimeId then
                local rec = R.WorldObjects[item.runtimeId]
                local obj = R.WorldInstanceByRuntimeId[item.runtimeId]

                if rec and not rec.removedAt and rec.periodicRefresh and obj and obj.Parent then
                    new[#new + 1] = {runtimeId = item.runtimeId}
                else
                    R.MemoryGuard.periodicRefreshEntriesDropped =
                        (R.MemoryGuard.periodicRefreshEntriesDropped or 0) + 1
                end
            end
        end

        R.PeriodicRefreshList = new
        R.PeriodicRefreshIndex = math.min(R.PeriodicRefreshIndex, math.max(1, #new))
    end
    ...
    local function trimHistoriesForMemory(level)
        ...
        for _,cluster in pairs(R.ResourceClusters) do
        ...
        while #R.BaseRegionModel.recentSamples > baseSampleLimit do
            table.remove(R.BaseRegionModel.recentSamples, 1)
        end
    end
    ...
    local function applyMemoryLevel(level)
        if level == "CRITICAL" then
            R.MemoryGuard.workFactor = 0.12
            R.MemoryGuard.logMultiplier = 6.0
            R.MemoryGuard.clusterMultiplier = 8.0
        elseif level == "HIGH" then
            R.MemoryGuard.workFactor = 0.30
            R.MemoryGuard.logMultiplier = 3.5
            R.MemoryGuard.clusterMultiplier = 4.0
        elseif level == "ELEVATED" then
            R.MemoryGuard.workFactor = 0.65
            R.MemoryGuard.logMultiplier = 1.75
            R.MemoryGuard.clusterMultiplier = 2.0
        else
            R.MemoryGuard.workFactor = 1.0
            R.MemoryGuard.logMultiplier = 1.0
            R.MemoryGuard.clusterMultiplier = 1.0
        end
    end

    local function runMemoryPrune(reason, forceCritical)
        local level = forceCritical and "CRITICAL" or R.MemoryGuard.level
        ...
        if level == "CRITICAL" and type(collectgarbage) == "function" then
            ...
            if ok then
                R.MemoryGuard.emergencyCollections =
                    (R.MemoryGuard.emergencyCollections or 0) + 1
            end
        end
        ...
        if worldPruned > 0 or guiPruned > 0 then
            ...
            print(
                "[STA Stage 1 v1.2.1] MEMORY_GUARD_PRUNE"
                .. " | Level=" .. tostring(level)
                .. " | World=" .. tostring(worldPruned)
                .. " | GUI=" .. tostring(guiPruned)
            )
    ...
    local function updateMemoryGuard(force)
        if not R.MemoryGuard.enabled then return end

        local t = now()
        if not force
            and (t - R.MemoryGuard.lastCheckAt) < CONFIG.MemoryCheckInterval
        then
            return
        end
        R.MemoryGuard.lastCheckAt = t

        local totalMb, source = readClientMemoryMb()
        R.MemoryGuard.source = source

        if type(totalMb) ~= "number" then
            return
        end

        R.MemoryGuard.totalMb = round(totalMb, 1)
        R.MemoryGuard.peakMb = math.max(R.MemoryGuard.peakMb or 0, totalMb)

        if R.MemoryGuard.baselineMb == nil or totalMb < R.MemoryGuard.baselineMb then
            R.MemoryGuard.baselineMb = totalMb
        end

        R.MemoryGuard.deltaMb =
            math.max(0, totalMb - (R.MemoryGuard.baselineMb or totalMb))

        local newLevel = memoryLevelFor(totalMb, R.MemoryGuard.baselineMb)

        if newLevel ~= R.MemoryGuard.level then
            local oldLevel = R.MemoryGuard.level
            R.MemoryGuard.level = newLevel
            R.MemoryGuard.transitions = (R.MemoryGuard.transitions or 0) + 1
            R.Statistics.memoryGuardTransitions =
                (R.Statistics.memoryGuardTransitions or 0) + 1
            ...
            logLine(
                ...
                "BaselineMb=" .. tostring(round(R.MemoryGuard.baselineMb, 1)),
                "DeltaMb=" .. tostring(round(R.MemoryGuard.deltaMb, 1)),
                "Source=" .. tostring(source)
            )

            print(
                "[STA Stage 1 v1.2.1] MEMORY_GUARD"
                .. " | " .. tostring(oldLevel) .. " -> " .. tostring(newLevel)
                .. " | Memory=" .. tostring(round(totalMb, 1)) .. " MB"
                .. " | Delta=" .. tostring(round(R.MemoryGuard.deltaMb, 1)) .. " MB"
            )
        else
            applyMemoryLevel(newLevel)
        end
        ...
        if newLevel ~= "NORMAL"
            and (force
                or (t - R.MemoryGuard.lastPruneAt) >= CONFIG.MemoryPruneInterval)
        then
            R.MemoryGuard.lastPruneAt = t
            runMemoryPrune("PERIODIC_PRESSURE", false)
        end
    end

    -- ============================================================
    -- DYNAMIC STREAMING QUEUE HELPERS
    -- ============================================================
    ...
    local function compactQueue(queue, head)
        ...
        -- Do not compact tiny queues every frame.
        if head < 256 and head <= (#queue / 2) then
            return queue, head
        end
    ...
    local function enqueueWorldObject(obj)
        if not obj then
            return false
        end

        -- Already registered: nothing to queue.
        if R.RuntimeByInstance[obj] then
            R.Statistics.dynamicWorldDuplicateSkips =
                (R.Statistics.dynamicWorldDuplicateSkips or 0) + 1
            return false
        end

        -- Already pending.
        if R.DynamicWorldQueued[obj] then
            R.Statistics.dynamicWorldDuplicateSkips =
                (R.Statistics.dynamicWorldDuplicateSkips or 0) + 1
            return false
        end

        R.DynamicWorldQueued[obj] = true
        R.DynamicWorldQueue[#R.DynamicWorldQueue + 1] = obj

        R.Statistics.dynamicWorldQueued =
            (R.Statistics.dynamicWorldQueued or 0) + 1

        local pending =
            math.max(0, #R.DynamicWorldQueue - R.DynamicWorldHead + 1)

        if pending > (R.Statistics.streamingBurstPeak or 0) then
            R.Statistics.streamingBurstPeak = pending
        end

        if pending > (R.StreamingBurstPeak or 0) then
            R.StreamingBurstPeak = pending
        end

        return true
    end

    local function enqueueGuiObject(obj)
        if not obj then
            return false
        end
        ...
        if not shouldTrackGui(obj) then
            return false
        end

        if R.GuiRuntimeByInstance[obj] then
            R.Statistics.dynamicGuiDuplicateSkips =
                (R.Statistics.dynamicGuiDuplicateSkips or 0) + 1
            return false
        end

        if R.DynamicGuiQueued[obj] then
            R.Statistics.dynamicGuiDuplicateSkips =
                (R.Statistics.dynamicGuiDuplicateSkips or 0) + 1
            return false
        end

        R.DynamicGuiQueued[obj] = true
        R.DynamicGuiQueue[#R.DynamicGuiQueue + 1] = obj

        R.Statistics.dynamicGuiQueued =
            (R.Statistics.dynamicGuiQueued or 0) + 1
    ...
    local function currentAdaptiveFactor()
        ...
        if DEVICE.Mobile then
            if R.FpsEma <= CONFIG.CriticalFpsThreshold then
                fpsFactor = 0.25
            elseif R.FpsEma <= CONFIG.LowFpsThreshold then
                fpsFactor = 0.50
            end
        else
            if R.FpsEma <= CONFIG.CriticalFpsThreshold then
        ...
        return math.max(
            0.08,
            fpsFactor * (R.MemoryGuard.workFactor or 1.0)
        )
    end
    ...
    local function processDynamicQueues()
        ...
        while R.DynamicWorldHead <= #R.DynamicWorldQueue and processedWorld < maxWorld do
            if safeClock() - started >= budgetSeconds then break end

            local obj = R.DynamicWorldQueue[R.DynamicWorldHead]
            R.DynamicWorldQueue[R.DynamicWorldHead] = nil
            R.DynamicWorldHead = R.DynamicWorldHead + 1

            if obj then
                R.DynamicWorldQueued[obj] = nil

                if R.Running
                    and obj.Parent
                    and not R.RuntimeByInstance[obj]
                    and shouldTrackObject(obj)
                then
                    local ok, err =
                        pcall(registerWorldObject, obj, false)
            ...
            R.Statistics.dynamicWorldProcessed = (R.Statistics.dynamicWorldProcessed or 0) + 1
        end

        local processedGui = 0
        while R.DynamicGuiHead <= #R.DynamicGuiQueue and processedGui < maxGui do
            if safeClock() - started >= budgetSeconds then break end

            local obj = R.DynamicGuiQueue[R.DynamicGuiHead]
            R.DynamicGuiQueue[R.DynamicGuiHead] = nil
            R.DynamicGuiHead = R.DynamicGuiHead + 1

            if obj then
                R.DynamicGuiQueued[obj] = nil

                if EXECUTOR.guiMode == "FULL_PLAYERGUI_BUDGETED"
                    and R.Running
                    and obj.Parent
                    and not R.GuiRuntimeByInstance[obj]
                    and shouldTrackGui(obj)
                then
                    local ok, err =
                        pcall(registerGui, obj, false)
            ...
            R.Statistics.dynamicGuiProcessed = (R.Statistics.dynamicGuiProcessed or 0) + 1
        end

        R.DynamicWorldQueue, R.DynamicWorldHead = compactQueue(R.DynamicWorldQueue, R.DynamicWorldHead)
        R.DynamicGuiQueue, R.DynamicGuiHead = compactQueue(R.DynamicGuiQueue, R.DynamicGuiHead)
    end

    local function processPeriodicRefreshBudget()
        local count = #R.PeriodicRefreshList
        ...
        while processed < maxObjects and checked < count do
            ...
            if R.PeriodicRefreshIndex > count then
                R.PeriodicRefreshIndex = 1
            end

            local item = R.PeriodicRefreshList[R.PeriodicRefreshIndex]
            R.PeriodicRefreshIndex = R.PeriodicRefreshIndex + 1
            checked = checked + 1
    ...
    local function scanWorkspaceInitial()
        ...
        while R.Running and head <= #queue do
            ...
            if parent then
                ...
                if okChildren and type(children) == "table" then
                    for _,child in ipairs(children) do
                        ...
                        if shouldTrackObject(child) and not R.RuntimeByInstance[child] then
    ...
    local function pollTargetedGuiState(forceLog)
        ...
        if dayCounter then
            ...
            if okText and type(text) == "string" then
                local oldText = R.TargetedGuiState.dayCounterText
                R.TargetedGuiState.dayCounterText = text
                R.TargetedGuiState.dayCounterPath = safeFullName(dayCounter)
                R.TargetedGuiState.observations =
                    (R.TargetedGuiState.observations or 0) + 1
                R.TargetedGuiState.lastObservedAt = elapsed()

                if oldText ~= nil and oldText ~= text then
                    R.TargetedGuiState.changes =
                        (R.TargetedGuiState.changes or 0) + 1
    ...
    local function scanGuiInitial()
        if EXECUTOR.guiMode == "TARGETED_STATE_ONLY" then
            ...
            print(
                "[STA Stage 1 v1.2.1] INITIAL_GUI_SCAN"
                ...
                if okChildren and type(children) == "table" then
                    for _,child in ipairs(children) do
                        ...
                        if shouldTrackGui(child)
                            and not R.GuiRuntimeByInstance[child]
                        then
                            local okRegister, registerErr =
    ...
    local function connectCharacterContext()
        if R.CharacterConnection then
            pcall(function() R.CharacterConnection:Disconnect() end)
            R.CharacterConnection = nil
        end
        R.CharacterConnection = LocalPlayer.CharacterAdded:Connect(function()
            safeWait(0.5)
            if R.Running then
                updateContext()
    ...
    local function updateMobileControlVisuals()
        if not R.MobileGui or not R.MobileGui.Parent then return end

        if R.MobileStatusLabel then
            if R.Running then
                R.MobileStatusLabel.Text = "STAGE 1: RUNNING"
            else
                R.MobileStatusLabel.Text = "STAGE 1: IDLE"
            end
        end

        if R.MobileStartButton then
            R.MobileStartButton.Text = R.Running and "RUNNING" or "START"
            R.MobileStartButton.AutoButtonColor = not R.Running
            R.MobileStartButton.Active = not R.Running
        end

        if R.MobileSaveButton then
            R.MobileSaveButton.Text = R.Running and "SAVE NOW" or "SAVE"
            R.MobileSaveButton.Active = true
        end

        if R.MobileStopButton then
            R.MobileStopButton.Text = R.Running and "STOP + SAVE" or "STOPPED"
            R.MobileStopButton.AutoButtonColor = R.Running
            R.MobileStopButton.Active = R.Running
        end
    end

    local function destroyOldMobileGui()
        if R.MobileUIConnection then
            pcall(function() R.MobileUIConnection:Disconnect() end)
            R.MobileUIConnection = nil
        end

        if R.MobileGui then
            pcall(function() R.MobileGui:Destroy() end)
            R.MobileGui = nil
        end
    ...
    local function createMobileControls()
        ...
        R.MobileStartButton = makeButton("Start", "START")
        R.MobileSaveButton = makeButton("Save", "SAVE NOW")
        R.MobileStopButton = makeButton("Stop", "STOP + SAVE")

        R.MobileStartButton.Activated:Connect(function()
            if R.Running then return end
            print("[STA Stage 1 v1.2.1] MOBILE_INPUT | START")
            local ok, err = pcall(startObserver)
            if not ok then reportRuntimeError("MOBILE_START", err) end
            updateMobileControlVisuals()
        end)

        R.MobileSaveButton.Activated:Connect(function()
            print("[STA Stage 1 v1.2.1] MOBILE_INPUT | SAVE_NOW")
            local ok, result = pcall(saveMemory)
            if not ok then
                reportRuntimeError("MOBILE_SAVE", result)
            elseif result then
                print("[STA Stage 1 v1.2.1] MOBILE_SAVE_OK")
            else
                warn("[STA Stage 1 v1.2.1] MOBILE_SAVE_NOT_VERIFIED")
            end
            updateMobileControlVisuals()
        end)

        R.MobileStopButton.Activated:Connect(function()
            if not R.Running then return end
            print("[STA Stage 1 v1.2.1] MOBILE_INPUT | STOP_SAVE")
        ...
        R.MobileUIConnection = UserInputService.InputChanged:Connect(function(input)
        ...
        updateMobileControlVisuals()
        print("[STA Stage 1 v1.2.1] MOBILE_UI_READY")
        return true, "OK"
    end
    ...
    startObserver = function()
        if R.Running then
            print("[STA Stage 1 v1.2.1] Observer is already running.")
            return
        end

        print("[STA Stage 1 v1.2.1] MODULE_START | EXECUTOR_PREFLIGHT")
        print(
            "[STA Stage 1 v1.2.1] EXECUTOR_PROFILE"
        ...
        for _, item in ipairs(preflight) do
            ...
            if ok then
                print("[STA Stage 1 v1.2.1] PREFLIGHT_PASS | " .. tostring(name))
            else
                preflightFails = preflightFails + 1
                warn("[STA Stage 1 v1.2.1] PREFLIGHT_WARN | " .. tostring(name))
            end
        end
        print(
            "[STA Stage 1 v1.2.1] MODULE_OK | EXECUTOR_PREFLIGHT"
            .. " | Warnings=" .. tostring(preflightFails)
        )

        local function checkpoint(name)
            print("[STA Stage 1 v1.2.1] START_CHECKPOINT | " .. tostring(name))
        end

        local function cleanupPartialStart()
            R.Running = false
            if R.MainHeartbeatConnection then
                pcall(function() R.MainHeartbeatConnection:Disconnect() end)
                R.MainHeartbeatConnection = nil
            end
            if R.CharacterConnection then
                pcall(function() R.CharacterConnection:Disconnect() end)
        ...
        local function startupBody()
            ...
            R.Running = true
            R.SessionStartClock = now()
            R.SessionUnix = safeUnixTime()
            R.LOG_FILE = string.format("STA_STAGE1_WORLD_V121_%d.log", R.SessionUnix)
            R.MEMORY_FILE = string.format("STA_STAGE1_WORLD_MEMORY_V121_%d.json", R.SessionUnix)
            ...
            R.GuiCounter = 0
            R.ClusterCounter = 0
            R.RuntimeByInstance = setmetatable({}, {__mode = "k"})
            R.GuiRuntimeByInstance = setmetatable({}, {__mode = "k"})
            R.WorldInstanceByRuntimeId = setmetatable({}, {__mode = "v"})
            R.GuiInstanceByRuntimeId = setmetatable({}, {__mode = "v"})
            R.ObjectConnectionsByRuntimeId = {}
            R.GuiConnectionsByRuntimeId = {}
            R.WorldObjects = {}
            R.GuiObjects = {}
            ...
            logLine(
                ...
                "WorldMode=" .. tostring(EXECUTOR.worldObservationMode),
            ...
            logLine("PERFORMANCE_COUNTER_SCHEMA", "v1.2.1", "NIL_SAFE=true")
            ...
            print("[STA Stage 1 v1.2.1] MODULE_START | INITIAL_SCAN_THREAD")
            local spawnOk, spawnErr = pcall(function()
                safeSpawn(function()
                    ...
                    print("[STA Stage 1 v1.2.1] MODULE_OK | INITIAL_SCAN_THREAD")
                end)
            end)
            if not spawnOk then
            ...
            R.LastObjectStateScan = now()
            R.LastClusterScan = now()
            R.LastTargetedGuiPoll = now()
            R.LastContextScan = now()
            R.LastAutosave = now()
            R.LastConsoleStatus = now()

            R.DynamicWorldQueue = {}
            R.DynamicWorldHead = 1
            R.DynamicWorldQueued = setmetatable({}, {__mode = "k"})
            R.DynamicGuiQueue = {}
            R.DynamicGuiHead = 1
            R.DynamicGuiQueued = setmetatable({}, {__mode = "k"})
            R.PeriodicRefreshIndex = 1
            R.ResourceClusterDirty = true
            R.LastClusterVerification = 0
            R.FpsEma = 60
            ...
            local okHeartbeat, heartbeatOrErr = pcall(function()
                return RunService.Heartbeat:Connect(function()
                    ...
                    local okTick, tickErr = pcall(function()
                        ...
                        if R.LastHeartbeatAt then
                            local dt = t - R.LastHeartbeatAt
                            if dt > 0 and dt < 1 then
                                local fps = math.min(240, 1 / dt)
                                R.FpsEma = (R.FpsEma * 0.92) + (fps * 0.08)
                            end
                        end
                        R.LastHeartbeatAt = t
                        ...
                        local effectiveObjectStateInterval =
                            CONFIG.ObjectStateInterval

                        if EXECUTOR.worldObservationMode == "POLLING_ONLY" then
                            effectiveObjectStateInterval =
                                math.min(
                                    effectiveObjectStateInterval,
                                    CONFIG.XenoPollingStateInterval
                                )
                        end

                        if t - R.LastObjectStateScan
                            >= effectiveObjectStateInterval
                        then
                            R.LastObjectStateScan = t
                            processPeriodicRefreshBudget()
                        end

                        local effectiveClusterInterval =
                            CONFIG.ClusterInterval
                            * (R.MemoryGuard.clusterMultiplier or 1)

                        if t - R.LastClusterScan >= effectiveClusterInterval then
                            local mustVerify = (t - R.LastClusterVerification) >= CONFIG.ClusterVerifyInterval
                            if R.ResourceClusterDirty or mustVerify then
                                R.LastClusterScan = t
                                R.LastClusterVerification = t
                                R.ResourceClusterDirty = false
                        ...
                        if t - R.LastContextScan >= CONFIG.PlayerContextInterval then
                            R.LastContextScan = t
                            protectedStage(
                        ...
                        if EXECUTOR.guiMode == "TARGETED_STATE_ONLY"
                            and (t - R.LastTargetedGuiPoll)
                                >= CONFIG.TargetedGuiPollInterval
                        then
                            R.LastTargetedGuiPoll = t
                        ...
                        if t - R.LastAutosave >= CONFIG.AutoSaveInterval then
                            R.LastAutosave = t

                            if R.MemoryGuard.level == "HIGH" then
                                runMemoryPrune("PRE_AUTOSAVE_HIGH", false)
                            elseif R.MemoryGuard.level == "CRITICAL" then
                        ...
                        if t - R.LastConsoleStatus >= CONFIG.ConsoleStatusInterval then
                            R.LastConsoleStatus = t
                            print(
                                "[STA Stage 1 v1.2.1] STATUS"
                                .. " | Runtime=" .. tostring(math.floor(elapsed()))
                                .. "s | Objects=" .. tostring(R.Statistics.objectsObserved)
                                .. " | Semantic=" .. tostring(R.Statistics.semanticCandidates)
                                .. " | Prompts=" .. tostring(R.Statistics.promptDiscovered)
                                .. " | GUI=" .. tostring(R.Statistics.guiObserved)
                                .. " | GuiMode=" .. tostring(EXECUTOR.guiMode)
                                .. " | WorldMode=" .. tostring(EXECUTOR.worldObservationMode)
                                .. " | Clusters=" .. tostring(R.Statistics.resourceClustersCreated)
                                .. " | FPS~" .. tostring(math.floor(R.FpsEma + 0.5))
                                .. " | WorldQueue=" .. tostring(math.max(0, #R.DynamicWorldQueue - R.DynamicWorldHead + 1))
                                .. " | GuiQueue=" .. tostring(math.max(0, #R.DynamicGuiQueue - R.DynamicGuiHead + 1))
                                .. " | BurstPeak=" .. tostring(R.Statistics.streamingBurstPeak or 0)
                                .. " | Reclass=" .. tostring(R.Statistics.semanticReclassifications or 0)
                                .. " | BaseSamples=" .. tostring(R.BaseRegionModel.sampleCount or 0)
                                .. " | Day=" .. tostring(R.CurrentContext.day)
                                .. " | DaySrc=" .. tostring(R.CurrentContext.daySource)
                                .. " | Mem=" .. tostring(R.MemoryGuard.totalMb or "?") .. "MB"
                                .. " | MemLevel=" .. tostring(R.MemoryGuard.level)
                                .. " | MemPeak=" .. tostring(round(R.MemoryGuard.peakMb or 0, 1)) .. "MB"
                                .. " | PrunedW=" .. tostring(R.MemoryGuard.worldRecordsPruned or 0)
                                .. " | PrunedG=" .. tostring(R.MemoryGuard.guiRecordsPruned or 0)
                                .. " | ThrottledLogs="
                                .. tostring(
                                    (R.Statistics.throttledNumericLogs or 0)
                                    + (R.Statistics.throttledGuiTextLogs or 0)
                                )
                            )
                        end
                    end)
            ...
            R.MainHeartbeatConnection = heartbeatOrErr

            checkpoint("11_STARTED")
            updateMobileControlVisuals()
            print("============================================================")
            print(" STA TRUE AI - STAGE 1 / 8")
            print(" SEMANTIC WORLD OBSERVER v1.2.1")
        ...
        if not ok then
            ...
            cleanupPartialStart()
            print("[STA Stage 1 v1.2.1] START FAILED - send the START_CHECKPOINT + ERROR lines.")
            return
        end
    end
    ...
    stopObserver = function()
        if not R.Running then
            print("[STA Stage 1 v1.2.1] Observer is not running.")
            return
        end
        ...
        logLine(
            "DISCOVERY_SUMMARY",
            "Objects=" .. tostring(R.Statistics.objectsObserved),
            "Preexisting=" .. tostring(R.Statistics.objectsPreexistingAtStart),
            "ObservedAfterStart=" .. tostring(R.Statistics.objectsObservedAfterStart),
            "Removed=" .. tostring(R.Statistics.objectsRemoved),
            "Reobserved=" .. tostring(R.Statistics.objectsReobserved),
            "Prompts=" .. tostring(R.Statistics.promptDiscovered),
            "GUI=" .. tostring(R.Statistics.guiObserved),
            "Clusters=" .. tostring(R.Statistics.resourceClustersCreated)
        )
        ...
        updateMobileControlVisuals()
        print("[STA Stage 1 v1.2.1] STOPPED")
        print("[STA Stage 1 v1.2.1] LOG: " .. R.LOG_FILE)
        if memorySaved then
            print("[STA Stage 1 v1.2.1] MEMORY_SAVE_VERIFIED: " .. R.MEMORY_FILE)
        else
            warn("[STA Stage 1 v1.2.1] MEMORY_NOT_VERIFIED: " .. R.MEMORY_FILE)
        end
    end

    -- ============================================================
    -- CRITICAL INTERNAL FUNCTION SELF-TEST
    -- ============================================================
    ...
    do
        local requiredFunctions = {
            ...
            scanWorkspaceInitial = scanWorkspaceInitial,
            scanGuiInitial = scanGuiInitial,
            enqueueWorldObject = enqueueWorldObject,
            enqueueGuiObject = enqueueGuiObject,
            compactQueue = compactQueue,
            processDynamicQueues = processDynamicQueues,
            processPeriodicRefreshBudget = processPeriodicRefreshBudget,
            updateMemoryGuard = updateMemoryGuard,
        }

        local missing = {}

        for name,fn in pairs(requiredFunctions) do
            if type(fn) ~= "function" then
                missing[#missing + 1] =
                    name .. "=" .. tostring(type(fn))
            end
        end

        if #missing > 0 then
            error(
                "STA Stage 1 internal bootstrap failure: "
                .. table.concat(missing, ", ")
            )
        end

        print(
            "[STA Stage 1 v1.2.1] INTERNAL_FUNCTION_SELF_TEST_OK"
            .. " | Count=" .. tostring(tableCount(requiredFunctions))
        )
    end
    ...
    do
        ...
        if startType == "function" and stopType == "function" then
            print(
                "[STA Stage 1 v1.2.1] MOBILE_CALLBACK_BINDING_OK"
                .. " | startObserver=" .. startType
                .. " | stopObserver=" .. stopType
            )
        else
            warn(
                "[STA Stage 1 v1.2.1] MOBILE_CALLBACK_BINDING_FAIL"
                .. " | startObserver=" .. startType
                .. " | stopObserver=" .. stopType
            )
        end
    ...
    ENV.STA_STAGE1_WORLD_OBJECTS = function() return R.WorldObjects end
    ENV.STA_STAGE1_GUI_OBJECTS = function() return R.GuiObjects end
    ENV.STA_STAGE1_SEMANTIC_CANDIDATES = function() return R.SemanticCandidates end
    ENV.STA_STAGE1_RESOURCE_CLUSTERS = function() return R.ResourceClusters end
    ENV.STA_STAGE1_MEMORY_GUARD = function() return R.MemoryGuard end
    ENV.STA_STAGE1_ARCHIVED_ENTITIES = function() return R.ArchivedWorldEntities end
    ENV.STA_STAGE1_BASE_INFRASTRUCTURE = function() return R.BaseInfrastructure end
    ENV.STA_STAGE1_LOOT_CONTAINERS = function() return R.LootContainers end
    ENV.STA_STAGE1_STATISTICS = function() return R.Statistics end
    ...
    ENV.STA_STAGE1_TARGETED_GUI_STATE =
        function() return R.TargetedGuiState end

    ENV.STA_STAGE1_INTERNAL_STATUS = function()
        return {
            version = VERSION,
            running = R.Running,
            executor = EXECUTOR,
            ...
            worldQueue =
                math.max(
                    0,
                    #R.DynamicWorldQueue - R.DynamicWorldHead + 1
                ),
            guiQueue =
                math.max(
                    0,
                    #R.DynamicGuiQueue - R.DynamicGuiHead + 1
                ),
            criticalFunctions = {
                enqueueWorldObject =
                    type(enqueueWorldObject),
                enqueueGuiObject =
                    type(enqueueGuiObject),
                compactQueue =
                    type(compactQueue),
                processDynamicQueues =
                    type(processDynamicQueues),
            },
        }
    end

    print("[STA Stage 1 v1.2.1] BOOTSTRAP_OK")
    ...
    do
        ...
        if okControl and connOrErr then
            R.ControlConnection = connOrErr
            ENV.STA_STAGE1_CONTROL_CONNECTION = R.ControlConnection
            print("[STA Stage 1 v1.2.1] MODULE_OK | CONTROL_BINDING")
        else
    ...
    if DEVICE.TouchEnabled then
        print("[STA Stage 1 v1.2.1] MODULE_START | MOBILE_TOUCH_UI")
        local okMobile, mobileResult, mobileDetail = pcall(createMobileControls)
        if okMobile and mobileResult then
            print("[STA Stage 1 v1.2.1] MODULE_OK | MOBILE_TOUCH_UI")
        else
            reportRuntimeError(
        ...
        print("[STA Stage 1 v1.2.1] MOBILE_TOUCH_UI | SKIPPED | TouchEnabled=false")
    end

    print("============================================================")
    print(" STA TRUE AI PROJECT - STAGE 1 / 8")
    print(" SEMANTIC WORLD OBSERVER v1.2.1 UNIVERSAL EXECUTOR")
    print(
        " EXECUTOR | " .. tostring(EXECUTOR.display)
        .. " | Family=" .. tostring(EXECUTOR.family)
        .. " | JSON=" .. tostring(EXECUTOR.jsonMode)
        .. " | GUI=" .. tostring(EXECUTOR.guiMode)
        .. " | WORLD=" .. tostring(EXECUTOR.worldObservationMode)
    )
    ...
    print(" XENO WORLD POLICY = SNAPSHOT + POLLING / NO PER-OBJECT SIGNAL STORM")
    print(" INITIAL SAVE = LIGHTWEIGHT CHECKPOINT; FULL SAVE ON DELETE/30MIN")
    print(" INTERNAL QUEUE SELF-TEST = ENABLED")
    print(" LOCAL REGISTER FIX = RUNTIME STATE NAMESPACED")
    print("============================================================")
    ...
    print("[STA Stage 1 v1.2.1] MODULE_OK | TOP_LEVEL_INIT")
end, __STA_STAGE1_FATAL_HANDLER)

if not __STA_STAGE1_TOP_OK then
    print("[STA Stage 1 v1.2.1] ABORTED_DURING_TOP_LEVEL_INIT")
end
