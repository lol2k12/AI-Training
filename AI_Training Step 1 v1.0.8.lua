--[[
    STA TRUE AI PROJECT
    STAGE 1 / 8 - SEMANTIC WORLD OBSERVER v1.1.4 - POTASSIUM HARDENED
...
print("[STA Stage 1 v1.1.4] FILE_ENTRY_OK")
...
local function __STA_STAGE1_FATAL_HANDLER(err)
    ...
    if type(warn) == "function" then
        warn("[STA Stage 1 v1.1.4] TOP_LEVEL_FATAL | " .. message)
    else
        print("[STA Stage 1 v1.1.4] TOP_LEVEL_FATAL | " .. message)
    end
    return message
end

local __STA_STAGE1_TOP_OK, __STA_STAGE1_TOP_ERR = xpcall(function()
    print("[STA Stage 1 v1.1.4] MODULE_START | TOP_LEVEL_INIT")
    ...
    local VERSION = "Stage1-v1.1.4"
    local SCHEMA = "STA True AI - Semantic World Observer Stage 1 v1.1.4"
    ...
    local function reportRuntimeError(stage, err)
        ...
        if t - state.windowStart >= 5 then
            if state.suppressed > 0 then
                warn(
                    "[STA Stage 1 v1.1.4] ERROR_SUPPRESSION_SUMMARY"
                    .. " | Stage=" .. stageText
    ...
    local function protectedStage(stage, fn)
        ...
        if not quiet then
            print("[STA Stage 1 v1.1.4] MODULE_START | " .. stageName)
        end
    ...
    local LOG_FILE = string.format("STA_STAGE1_WORLD_V114_%d.log", SessionUnix)
    local MEMORY_FILE = string.format("STA_STAGE1_WORLD_MEMORY_V114_%d.json", SessionUnix)
    ...
    local function saveMemory()
        if not canWrite then
            ...
            warn("[STA Stage 1 v1.1.4] MEMORY_SAVE_FAIL | " .. msg)
            return false
        end
        ...
        if okEncode and type(encodedOrErr) == "string" then
            ...
            warn(
                "[STA Stage 1 v1.1.4] JSON_HTTP_ENCODER_REJECTED"
                .. " | " .. httpErr
            ...
            if okCustom and type(customOrErr) == "string" then
                ...
                print(
                    "[STA Stage 1 v1.1.4] JSON_CUSTOM_ENCODER_OK"
                ...
                if type(HttpService.JSONDecode) == "function" then
                    ...
                    if okDecode then
                        print("[STA Stage 1 v1.1.4] JSON_CUSTOM_VALIDATION_OK")
                    else
                ...
                for i,sample in ipairs(JsonSanitizeStats.samples) do
                    warn(
                        "[STA Stage 1 v1.1.4] JSON_SANITIZE_SAMPLE | #" .. tostring(i)
        ...
        print("[STA Stage 1 v1.1.4] JSON_ENCODER_USED | " .. tostring(encoderUsed))
        ...
        if JsonSanitizeStats.converted > 0 then
            ...
            print(
                "[STA Stage 1 v1.1.4] JSON_SANITIZE_SUMMARY"
                .. " | Converted=" .. tostring(JsonSanitizeStats.converted)
                .. " | NonFinite=" .. tostring(JsonSanitizeStats.nonFiniteNumbers)
                .. " | Unsupported=" .. tostring(JsonSanitizeStats.unsupportedValues)
                .. " | Cycles=" .. tostring(JsonSanitizeStats.cycles)
        ...
        if verified then
            ...
            print("[STA Stage 1 v1.1.4] MEMORY_SAVE_OK | " .. MEMORY_FILE .. " | Bytes=" .. tostring(#encoded) .. " | Encoder=" .. tostring(encoderUsed))
            return true
        end

        logLine("MEMORY_SAVE_FAIL", MEMORY_FILE, verifyDetail)
        warn("[STA Stage 1 v1.1.4] MEMORY_SAVE_FAIL | " .. MEMORY_FILE .. " | " .. verifyDetail)
        return false
    end
    ...
    local function enqueueWorldObject(obj)
        ...
        if RuntimeByInstance[obj] or DynamicWorldQueued[obj] then
        ...
        Statistics.dynamicWorldQueued = (Statistics.dynamicWorldQueued or 0) + 1
    ...
    local function enqueueGuiObject(obj)
        ...
        if GuiRuntimeByInstance[obj] or not shouldTrackGui(obj) or DynamicGuiQueued[obj] then
        ...
        Statistics.dynamicGuiQueued = (Statistics.dynamicGuiQueued or 0) + 1
    end
    ...
    local function createMobileControls()
        ...
        MobileStartButton.Activated:Connect(function()
            ...
            print("[STA Stage 1 v1.1.4] MOBILE_INPUT | START")
        ...
        MobileSaveButton.Activated:Connect(function()
            print("[STA Stage 1 v1.1.4] MOBILE_INPUT | SAVE_NOW")
            local ok, result = pcall(saveMemory)
            if not ok then
                reportRuntimeError("MOBILE_SAVE", result)
            elseif result then
                print("[STA Stage 1 v1.1.4] MOBILE_SAVE_OK")
            else
                warn("[STA Stage 1 v1.1.4] MOBILE_SAVE_NOT_VERIFIED")
            end
            updateMobileControlVisuals()
        end)
        ...
        MobileStopButton.Activated:Connect(function()
            if not Running then return end
            print("[STA Stage 1 v1.1.4] MOBILE_INPUT | STOP_SAVE")
        ...
        updateMobileControlVisuals()
        print("[STA Stage 1 v1.1.4] MOBILE_UI_READY")
        return true, "OK"
    end
    ...
    startObserver = function()
        if Running then
            print("[STA Stage 1 v1.1.4] Observer is already running.")
            return
        end

        print("[STA Stage 1 v1.1.4] MODULE_START | POTASSIUM_PREFLIGHT")
        local preflight = {
            {"Players", Players ~= nil},
        ...
        for _, item in ipairs(preflight) do
            ...
            if ok then
                print("[STA Stage 1 v1.1.4] PREFLIGHT_PASS | " .. tostring(name))
            else
                preflightFails = preflightFails + 1
                warn("[STA Stage 1 v1.1.4] PREFLIGHT_WARN | " .. tostring(name))
            end
        end
        print("[STA Stage 1 v1.1.4] MODULE_OK | POTASSIUM_PREFLIGHT | Warnings=" .. tostring(preflightFails))

        local function checkpoint(name)
            print("[STA Stage 1 v1.1.4] START_CHECKPOINT | " .. tostring(name))
        end
        ...
        local function startupBody()
            ...
            LOG_FILE = string.format("STA_STAGE1_WORLD_V114_%d.log", SessionUnix)
            MEMORY_FILE = string.format("STA_STAGE1_WORLD_MEMORY_V114_%d.json", SessionUnix)
            ...
            logLine("PERFORMANCE_COUNTER_SCHEMA", "v1.1.4", "NIL_SAFE=true")
            ...
            print("[STA Stage 1 v1.1.4] MODULE_START | INITIAL_SCAN_THREAD")
            local spawnOk, spawnErr = pcall(function()
                safeSpawn(function()
                    ...
                    print("[STA Stage 1 v1.1.4] MODULE_OK | INITIAL_SCAN_THREAD")
                end)
            end)
            if not spawnOk then
            ...
            print(" SEMANTIC WORLD OBSERVER v1.1.4")
        ...
        if not ok then
            ...
            cleanupPartialStart()
            print("[STA Stage 1 v1.1.4] START FAILED - send the START_CHECKPOINT + ERROR lines.")
            return
        end
    end
    ...
    stopObserver = function()
        if not Running then
            print("[STA Stage 1 v1.1.4] Observer is not running.")
            return
        end
        ...
        updateMobileControlVisuals()
        print("[STA Stage 1 v1.1.4] STOPPED")
        print("[STA Stage 1 v1.1.4] LOG: " .. LOG_FILE)
        if memorySaved then
            print("[STA Stage 1 v1.1.4] MEMORY_SAVE_VERIFIED: " .. MEMORY_FILE)
        else
            warn("[STA Stage 1 v1.1.4] MEMORY_NOT_VERIFIED: " .. MEMORY_FILE)
        end
    end
    ...
    do
        ...
        if startType == "function" and stopType == "function" then
            print(
                "[STA Stage 1 v1.1.4] MOBILE_CALLBACK_BINDING_OK"
                .. " | startObserver=" .. startType
                .. " | stopObserver=" .. stopType
            )
        else
            warn(
                "[STA Stage 1 v1.1.4] MOBILE_CALLBACK_BINDING_FAIL"
                .. " | startObserver=" .. startType
                .. " | stopObserver=" .. stopType
            )
        end
    ...
    print("[STA Stage 1 v1.1.4] BOOTSTRAP_OK")
    ...
    do
        ...
        if okControl and connOrErr then
            ...
            print("[STA Stage 1 v1.1.4] MODULE_OK | CONTROL_BINDING")
        else
    ...
    if DEVICE.TouchEnabled then
        print("[STA Stage 1 v1.1.4] MODULE_START | MOBILE_TOUCH_UI")
        local okMobile, mobileResult, mobileDetail = pcall(createMobileControls)
        if okMobile and mobileResult then
            print("[STA Stage 1 v1.1.4] MODULE_OK | MOBILE_TOUCH_UI")
        else
            reportRuntimeError(
        ...
        print("[STA Stage 1 v1.1.4] MOBILE_TOUCH_UI | SKIPPED | TouchEnabled=false")
    end

    print("============================================================")
    print(" STA TRUE AI PROJECT - STAGE 1 / 8")
    print(" SEMANTIC WORLD OBSERVER v1.1.4 UNIVERSAL")
    ...
    print("[STA Stage 1 v1.1.4] MODULE_OK | TOP_LEVEL_INIT")
end, __STA_STAGE1_FATAL_HANDLER)

if not __STA_STAGE1_TOP_OK then
    print("[STA Stage 1 v1.1.4] ABORTED_DURING_TOP_LEVEL_INIT")
end
